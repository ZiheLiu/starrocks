...
##### statistic_max_changes_rows_estimate_ratio

- Default: 2.0
- Type: Double
- Unit: -
- Is mutable: Yes
- Description: The maximum multiplier applied to the base row count when capping the estimated output row count for `CHANGES` queries. StarRocks estimates the `CHANGES` scan row count from the base table row count and version span, then limits the estimate to `base_row_count * statistic_max_changes_rows_estimate_ratio`. Set a smaller value to reduce overly large `CHANGES` cardinality estimates. Valid values: greater than or equal to `0`. This item takes effect immediately and does not require an FE restart.
- Introduced in: -
