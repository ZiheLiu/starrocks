...
- 单位：-
- 是否动态：是
- 描述：用于限制 `CHANGES` 查询估算输出行数上限的倍数。StarRocks 会先根据基础表行数和版本跨度估算 `CHANGES` scan 的行数，再将估算结果限制在 `base_row_count * statistic_max_changes_rows_estimate_ratio` 以内。您可以调小该值以避免 `CHANGES` 基数估算过大。取值范围：大于等于 `0`。该参数修改后立即生效，无需重启 FE。
- 引入版本：-
