...
public class SlotSelectionStrategyV2WeightedRoundRobinQueueTest {
    ...
    public void testAddAndPeakByPriority() {
        ...
        final int[] expectedPeakTimes = new int[] {16384, 4096, 1024, 256, 64, 16, 4, 1};
    ...
    @Test
    public void testPauseAndResumeSubQueue() {
        WeightedRoundRobinQueue queue = new WeightedRoundRobinQueue(1024, 1);
        ...
        // Restore the queue of slot2 and slot2 > slot1 (peak)
        queue.add(slot2);
        assertThat(queue.peak()).isSameAs(slot2);

        // Restore the queue of slot4 and slot4 > slot2 (peak)
        queue.add(slot4);
        assertThat(queue.peak()).isSameAs(slot4);

        // Restore the queue of slot3 but slot3 < slot4 (peak)
        queue.add(slot3);
        assertThat(queue.peak()).isSameAs(slot4);
