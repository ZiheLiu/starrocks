...
public class IvmVersionProjectRule extends TransformationRule {
    ...
    public List<OptExpression> transform(OptExpression input, OptimizerContext context) {
        ...
        Map<ColumnRefOperator, ScalarOperator> rewrittenProjectMap = Maps.newHashMap(project.getColumnRefMap());
        ...
        if (version.getActionColumn() != null) {
            ...
            rewrittenProjectMap.put(version.getActionColumn(), childActionColumn);
        }

        OptExpression projectChild = input.inputAt(0).inputAt(0);
        OptExpression versionChild = OptExpression.create(
                new LogicalVersionOperator(version.getTableVersion(), version.getAction(), childActionColumn), projectChild);
        LogicalProjectOperator rewrittenProject = LogicalProjectOperator.builder()
                .withOperator(project)
                .setColumnRefMap(rewrittenProjectMap)
                .build();
        return List.of(OptExpression.create(rewrittenProject, versionChild));
    }
}
