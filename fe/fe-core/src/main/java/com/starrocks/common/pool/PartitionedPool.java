...
public class PartitionedPool<V extends TServiceClient> implements GenericPool<Poolable<V>> {
    private final DefaultGenericPool<V>[] partitionPools;

    public PartitionedPool(String className, GenericKeyedObjectPoolConfig config, int timeoutMs, int numPartitions) {
        partitionPools = new DefaultGenericPool[numPartitions];
        for (int i = 0; i < numPartitions; i++) {
            partitionPools[i] = new DefaultGenericPool<>(className, config, timeoutMs);
        }
    }
    ...
    public void clearPool(TNetworkAddress addr) {
        for (DefaultGenericPool<V> pool : partitionPools) {
            pool.clearPool(addr);
        }
    }
