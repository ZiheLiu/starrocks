...
public class FrontendServiceProxy {
    ...
    public static <T> T callWithPartition(TNetworkAddress address, int timeoutMs, int retryTimes, MethodCallable<T> callable)
            throws Exception {
        Poolable<FrontendService.Client> client = ClientPool.slotPool.borrowObject(address, timeoutMs);
        boolean isConnValid = false;
        try {
            for (int i = 0; i < retryTimes; i++) {
                try {
                    T t = callable.invoke(client.getObject());
                    isConnValid = true;
                    return t;
                    ...
                    isConnValid = ClientPool.slotPool.reopen(client, timeoutMs);
                    if (i == retryTimes - 1 ||
                            !isConnValid ||
            ...
            if (isConnValid) {
                ClientPool.slotPool.returnObject(address, client);
            } else {
                ClientPool.slotPool.invalidateObject(address, client);
            }
        }
