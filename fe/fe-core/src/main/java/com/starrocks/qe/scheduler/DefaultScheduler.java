...
import com.starrocks.qe.scheduler.slot.DefaultSlotAllocator;
import com.starrocks.qe.scheduler.slot.SharingSlotRequirement;
import com.starrocks.qe.scheduler.slot.SlotAllocator;
...
public class DefaultScheduler implements ICoordinator, StateContext {
    ...
    static {
        STATE_MACHINE_FACTORY = new StateMachineFactory(new CreatedState())
                ...
                .addTransition(CreatedState.class, JobEventType.CANCEL, CancelledState.class, new CancelledState.GotoCancelled())
                .addTransition(CreatedState.class, JobEventType.FAIL, FailedState.class, new FailedState.GotoFailed())
                .addTransition(CreatedState.class, JobEventType.FINISHED, FailedState.class, new FailedState.GotoFailed())
                ...
                .addTransition(ExecutingState.class, JobEventType.CANCEL, CancellingState.class,
                        new CancellingState.GotoCancelling())
                .addTransition(ExecutingState.class, JobEventType.FAIL, FailingState.class, new FailingState.GotoFailing())
                .addTransition(ExecutingState.class, JobEventType.FINISHED, FinishedState.class, new FinishedState.GotoFinished())
                // CANCELLING
                .addTransition(CancellingState.class, JobEventType.FINISHED, CancelledState.class,
                        new CancelledState.GotoCancelled())
                // FAILING
                .addTransition(FailingState.class, JobEventType.FINISHED, FailedState.class, new FailedState.GotoFailed())
                // FINISHING
                .addTransition(FinishingState.class, JobEventType.FINISHED, FinishedState.class,
                        new FinishedState.GotoFinished())

                // RESET Event
                .addTransition(ImmutableSet.of(WaitingForResourceState.class, ExecutingState.class, CancellingState.class,
    ...
    private SharingSlotRequirement slotRequirement = null;
    ...
    public DefaultScheduler(ConnectContext context, JobInformation jobInformation, boolean needReport) {
        ...
        this.jobInformation = jobInformation;
        this.executionDAG = ExecutionDAG.build(jobInformation);
        this.slotAllocator = new DefaultSlotAllocator();
    ...
    public void updateFragmentExecStatus(TReportExecStatusParams params) {
        ...
        SchedulerTraceUtil.log(jobInformation, "updateFragmentExecStatus: receive [execution={}] [request={status={}, done={}}]",
    ...
    private void onExecutionFailed(ExecutionFragmentInstance execution, Status executionStatus) {
        SchedulerTraceUtil.log(jobInformation, "updateFragmentExecStatus: onExecutionFailed [execution={}] [executionStatus={}]",
    ...
    private void handleStatus(Status status) throws RpcException, UserException {
        switch (status.getErrorCode()) {
            ...
            case THRIFT_RPC_ERROR:
                LOG.warn("query rpc failed [query_id={}] [status={}]", DebugUtil.printId(jobInformation.getQueryId()), status,
                        failure);
            ...
            case TIMEOUT:
            default:
                LOG.warn("query failed [query_id={}] [status={}]", DebugUtil.printId(jobInformation.getQueryId()), status,
                        failure);
    ...
    public RowBatch getNext() throws Exception {
        ...
        if (!status.ok()) {
            ...
            boolean reachLimit = !jobInformation.isBlockQuery() && executionDAG.getInstanceIds().size() > 1 && hasLimit &&
            ...
            SchedulerTraceUtil.log(jobInformation, "getNext: success [curRows={}] [totalRows={}]", curNumRows, numReceivedRows);
        }
    ...
    public void requireSlot() {
        try (PlannerProfile.ScopedTimer ignore = PlannerProfile.getScopedTimer("Scheduler.Queued")) {
            try {
                slotRequirement = slotAllocator.calculateRequirement(jobInformation, workerProvider);
            } catch (SchedulerException e) {
                LOG.warn("Failed to calculate slot requirement", e);
                onFailure(Status.createInternalError(e.getMessage()), e);
                return;
            }

            SchedulerTraceUtil.log(jobInformation, "slot: require to allocate [slotRequirement={}]", slotRequirement);
            slotAllocator.requireSharingSlotAsync(slotRequirement);

            SchedulerTraceUtil.log(jobInformation, "slot: wait for allocating");
            Status status = slotRequirement.waitAllocated();
            SchedulerTraceUtil.log(jobInformation, "slot: wait finished [status={}]", status);
        }

        if (status.ok()) {
            workerProvider.acceptSlotRequirement(slotRequirement);
            onResourceReady();
        } else if (status.isCancelled()) {
            LOG.warn("Cancelled when waiting slot to be allocated: {}", status.getErrorMsg());
            // TODO(lzh): comment
            // Already cancelled.
        } else {
            LOG.warn("Failed to allocate slot: {}", status.getErrorMsg());
            onFailure(status);
        }
    }
    ...
    public void releaseSharingSlot() {
        if (slotRequirement != null) {
            slotAllocator.releaseSharingSlot(slotRequirement);
        }
    }

    @Override
    public void cancelSlotRequirement() {
        SharingSlotRequirement slotRequirement = this.slotRequirement;
        SchedulerTraceUtil.log(jobInformation, "slot: cancel [slotRequirement={}]", slotRequirement);
        if (slotRequirement != null) {
            slotRequirement.onCancel();
        }
    }
    ...
    public void initializeExecutionDAG() throws UserException {
        ...
        for (ExecutionFragment execFragment : executionDAG.getFragmentsInPreorder()) {
            executionDAG.initFragment(execFragment, workerProvider);
        }

        executionDAG.finalizeDAG(workerProvider);
    }
    ...
    public void initializeResultSink() throws AnalysisException {
        ...
        ComputeNode rootWorker = workerProvider.getUsingWorkerByAddr(rootInstance.getAddress());
        ...
        new GlobalRuntimeFilterInitializer(executionDAG, workerProvider, connectContext,
                jobInformation.isEnablePipeline()).setGlobalRuntimeFilterParams(rootFragment, rootWorker.getBrpcAddress());

        if (LOG.isDebugEnabled()) {
            LOG.debug("dispatch query job: {} to {}", DebugUtil.printId(jobInformation.getQueryId()),
                    rootFragment.getInstances().get(0).getAddress());
        }

        // set the broker address for OUTFILE sink
        if (resultSink.isOutputFileSink() && resultSink.needBroker()) {
            FsBroker broker =
                    GlobalStateMgr.getCurrentState().getBrokerMgr().getBroker(resultSink.getBrokerName(), rootWorker.getHost());
    ...
    public void initializeProfile() {
        if (jobInformation.isLoadType()) {
            ...
            GlobalStateMgr.getCurrentState().getLoadMgr()
                    .initJobProgress(jobInformation.getLoadJobId(), jobInformation.getQueryId(), executionDAG.getInstanceIds(),
                            workerProvider.getUsingWorkerIDs());
            LOG.info("dispatch load job: {} to {}", DebugUtil.printId(jobInformation.getQueryId()),
                    workerProvider.getUsingWorkerAddrs());
        }
    ...
    public synchronized void deploy() {
        ...
        Deployer deployer =
                new Deployer(connectContext, jobInformation, workerProvider, executionDAG, schedulerAddress, this::onFailure);
    ...
    public boolean checkBackendState() {
        for (ExecutionFragmentInstance execution : executionDAG.getNeedCheckExecutions()) {
            if (!execution.isBackendStateHealthy()) {
                setStatus(new Status(TStatusCode.INTERNAL_ERROR, "backend " + execution.getBackend().getId() + " is down"));
                return false;
            }
        }
        return true;
    ...
    public boolean isUsingBackend(Long backendID) {
        return workerProvider.isUsingWorker(backendID);
    }
    ...
    private void cancelRemoteFragmentsAsync(PPlanFragmentCancelReason cancelReason) {
        for (ExecutionFragmentInstance execution : executionDAG.getExecutions()) {
            if (!execution.cancelFragmentInstance(cancelReason) && (!execution.hasBeenDeployed() || execution.isFinished())) {
                // TODO(lzh): add comment
                executionDAGProfile.markedCountDown(execution.getFragmentInstanceId());
            }
        }
