...
public class DefaultScheduler implements ICoordinator, StateContext {
    ...
    public DefaultScheduler(ConnectContext context, JobInformation jobInfo, boolean needReport) {
        ...
        this.fragmentAssignmentStrategyFactory =
                new FragmentAssignmentStrategyFactory(connectContext, executionDAG, jobInfo.isEnablePipeline());
        ...
        this.executionDAGProfile = new ExecutionDAGProfile(connectContext, executionDAG,
                needReport || context.getSessionVariable().isEnableProfile(), jobInfo.getQueryId(),
    ...
    public void startScheduling(boolean needDeploy) throws RpcException, UserException {
        ...
        SchedulerTraceUtil.log(jobInfo, "startScheduling [jobInfo={}]", jobInfo);
    ...
    public void cancel(PPlanFragmentCancelReason reason, String message) {
        LOG.warn("cancel execution of query, this is outside invoke [queryId={}] [message={}]",
                DebugUtil.printId(jobInfo.getQueryId()), message);
            ...
            if (loadJobType == TLoadJobType.BROKER || loadJobType == TLoadJobType.INSERT_QUERY ||
                ...
                if (params.isSetSink_load_bytes() && params.isSetSource_load_rows() &&
                    ...
                    GlobalStateMgr.getCurrentState().getLoadMgr()
                            .updateJobPrgress(jobInfo.getLoadJobId(), params);
                }
            }
        } else {
            if (params.isSetSink_load_bytes() && params.isSetSource_load_rows() && params.isSetSource_load_bytes()) {
                GlobalStateMgr.getCurrentState().getLoadMgr().updateJobPrgress(jobInfo.getLoadJobId(), params);
            }
        }
    }
    ...
    private void handle(JobEvent event) {
        ...
        synchronized (this) {
            ...
            SchedulerTraceUtil.log(jobInfo, "state transition: receive event [{}] on state [{}]", event,
                    oldState.getType());
            ...
            try {
                ...
                // Do nothing.
                SchedulerTraceUtil.log(jobInfo, "state transition: event [{}] on state [{}] does nothing", event,
                        oldState.getType());
            }
        }

        if (oldState != newState && newState != null) {
            SchedulerTraceUtil.log(jobInfo, "state transition: change state from [{}] to [{}] on event [{}]",
    ...
    private void handleStatus(Status status) throws RpcException, UserException {
        switch (status.getErrorCode()) {
            ...
            case THRIFT_RPC_ERROR:
                LOG.warn("query rpc failed [query_id={}] [status={}]", DebugUtil.printId(jobInfo.getQueryId()),
                        status,
                        failure);
            ...
            case TIMEOUT:
            default:
                LOG.warn("query failed [query_id={}] [status={}]", DebugUtil.printId(jobInfo.getQueryId()),
                        status,
                        failure);
    ...
    public RowBatch getNext() throws Exception {
        if (receiver == null) {
            SchedulerTraceUtil.log(jobInfo, "getNext: receiver is null");
        ...
        if (!status.ok()) {
            SchedulerTraceUtil.log(jobInfo, "getNext: fail [status={}]", status);
            ...
            boolean reachLimit =
                    !jobInfo.isBlockQuery() && executionDAG.getInstanceIds().size() > 1 && hasLimit &&
                            numReceivedRows >= numLimitRows;
            SchedulerTraceUtil.log(jobInfo, "getNext: eos [reachLimit={}]", reachLimit);
            onFinishing(reachLimit);
        } else {
            int curNumRows = resultBatch.getBatch().getRowsSize();
            numReceivedRows += curNumRows;
            SchedulerTraceUtil.log(jobInfo, "getNext: success [curRows={}] [totalRows={}]", curNumRows,
                    numReceivedRows);
        }
    ...
    public void initializeResultSink() throws AnalysisException {
        ...
        GlobalRuntimeFilterInitializer grfInitializer = new GlobalRuntimeFilterInitializer(executionDAG, connectContext,
                jobInfo.isEnablePipeline());
        grfInitializer.setGlobalRuntimeFilterParams(rootFragment, rootWorker.getBrpcAddress());

        if (LOG.isDebugEnabled()) {
            LOG.debug("dispatch query job: {} to {}", DebugUtil.printId(jobInfo.getQueryId()),
    ...
    public void initializeProfile() {
        if (jobInfo.isLoadType()) {
            jobInfo.getQueryOptions().setEnable_profile(true);
            GlobalStateMgr.getCurrentState().getLoadMgr()
    ...
    public synchronized void deploy() {
        SchedulerTraceUtil.log(jobInfo, "deploy: [executionDAG={}]", executionDAG);
    ...
    public void endProfile() {
        SchedulerTraceUtil.log(jobInfo, "endProfile [profile={}]", executionDAGProfile);
        if (needDeploy) {
    ...
    public boolean join(int timeoutSecond) {
        ...
        while (leftTimeoutSecond > 0) {
            ...
            SchedulerTraceUtil.log(jobInfo, "join: [waitSecond={}]", waitSecond);
            if (executionDAGProfile.join(waitSecond)) {
                SchedulerTraceUtil.log(jobInfo, "join: finish");
                return true;
            }

            if (!checkBackendState()) {
                SchedulerTraceUtil.log(jobInfo, "join: some backend is unhealthy");
                return true;
            }
    ...
    public long getStartTimeMs() {
        return jobInfo.getQueryGlobals().getTimestamp_ms();
    }
