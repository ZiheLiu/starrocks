...
public class ExecutionDAG {
    ...
    private Map<TNetworkAddress, Integer> addrToNumInstances = Maps.newHashMap();
    ...
    public void initFragment(ExecutionFragment execFragment, WorkerProvider workerProvider) throws SchedulerException {
        ...
        if (execFragment.getPlanFragment() instanceof MultiCastPlanFragment) {
            initMultiCastFragment(execFragment, (MultiCastPlanFragment) execFragment.getPlanFragment(), workerProvider);
        } else {
            initNormalFragment(execFragment, workerProvider);
        }
    }
    ...
    private boolean needScheduleByBucketShuffleJoin(ExecutionFragment execFragment, DataSink sink) {
        if (execFragment.isBucketShuffleJoin()) {
    ...
    private void initMultiCastFragment(ExecutionFragment execFragment, MultiCastPlanFragment fragment,
                                       WorkerProvider workerProvider) throws SchedulerException {
        ...
        for (int i = 0; i < fragment.getDestFragmentList().size(); i++) {
            ...
            if (needScheduleByBucketShuffleJoin(destExecFragment, sink)) {
                ...
                for (FragmentInstance destInstance : destExecFragment.getInstances()) {
                    ...
                    ComputeNode worker = workerProvider.getUsingWorkerByAddr(destInstance.getAddress());
                    // NOTE(zc): can be removed in version 4.0
                    dest.setDeprecated_server(worker.getBeRpcAddress());
    ...
    private void initNormalFragment(ExecutionFragment execFragment, WorkerProvider workerProvider) {
    ...
    public void finalizeDAG(WorkerProvider workerProvider) {
        addrToNumInstances = fragments.stream()
                .flatMap(fragment -> fragment.getInstances().stream())
                .collect(Collectors.groupingBy(
                        FragmentInstance::getAddress,
            ...
            fragments.stream()
                    ...
                    .forEach(instance -> Stream.concat(
                            ...
                            .forEach(scanRange -> {
                                ...
                                TNetworkAddress addr = instance.getAddress();
                                TNetworkAddress beHttpAddress = workerProvider.getUsingHttpAddrByAddr(addr);
                                channelIdToBEHTTP.put(channelId, beHttpAddress);
                                channelIdToBEPort.put(channelId, addr);
                            })
                    );
        }
    }

    public int getNumInstancesOfAddr(TNetworkAddress addr) {
        return addrToNumInstances.get(addr);
    }
