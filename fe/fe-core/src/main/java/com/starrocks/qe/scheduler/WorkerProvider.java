...
public class WorkerProvider {
    ...
    /**
     * Filter the available workers by the allocated slots.
     *
     * @param slotRequirement The allocated slots.
     */
    public void acceptSlotRequirement(SharingSlotRequirement slotRequirement) {
        availableID2Backend = filterBySlotRequirement(availableID2Backend, slotRequirement);
        availableID2ComputeNode = filterBySlotRequirement(availableID2ComputeNode, slotRequirement);

        hasComputeNode = MapUtils.isNotEmpty(availableID2ComputeNode);
        usedComputeNode = hasComputeNode && usedComputeNode;
    }
    ...
    private static <C extends ComputeNode> ImmutableMap<Long, C> filterBySlotRequirement(
            ImmutableMap<Long, C> workers, SharingSlotRequirement slotRequirement) {
        if (slotRequirement.isEmpty()) {
            return workers;
        }

        return ImmutableMap.copyOf(
                workers.entrySet().stream()
                        .filter(entry -> slotRequirement.containsWorker(entry.getKey()))
                        .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue))
        );
    }
