...
public class FragmentInstance {
    ...
    public FragmentInstance(ComputeNode worker, TNetworkAddress address, ExecutionFragment execFragment) {
        this.worker = worker;
    ...
    public void buildExplainString(ExplainBuilder builder) {
        builder.addMultipleChildren("INSTANCE(" + getReadableId() + ")", () -> {
            if (execution != null) {
                ...
                if (!destinations.isEmpty()) {
                    ...
                    builder.addMultipleChildren("DESTINATIONS", () -> {
                        List<String> items = destinations.stream().map(destination -> {
                        ...
                        builder.addInlineItems(items);
                    });
                }
            }

            builder.addSimpleKVItem("BE", worker.getId());

            if (isSetPipelineDop()) {
                builder.addSimpleKVItem("DOP", pipelineDop);
            }
    ...
    private void explainScanRangesWithoutDriverSeq(ExplainBuilder builder) {
        builder.addMultipleChildren("SCAN RANGES", () -> {
            if (!bucketSeqToDriverSeq.isEmpty()) {
                builder.addMultipleChildren("BUCKET SEQUENCES", () -> builder.addInlineItems(bucketSeqToDriverSeq.keySet()));

            }

            node2ScanRanges.forEach((scanId, scanRanges) -> {
                ScanNode scanNode = execFragment.getScanNode(new PlanNodeId(scanId));
                builder.addMultipleChildren(scanId + ":" + scanNode.getPlanNodeName(),
    ...
    private void explainScanRangesWithDriverSeq(ExplainBuilder builder) {
        builder.addMultipleChildren("SCAN RANGES (per driver sequence)", () -> {
            if (!bucketSeqToDriverSeq.isEmpty()) {
                builder.addMultipleChildren("BUCKET SEQUENCE TO DRIVER SEQUENCE", () ->
                        builder.addInlineItems(bucketSeqToDriverSeq.entrySet().stream()
            ...
            node2DriverSeqToScanRanges.forEach((scanId, driverSeqToScanRanges) -> {
                ...
                builder.addMultipleChildren(scanId + ":" + scanNode.getPlanNodeName(), () -> {
                    ...
                    for (Map.Entry<Integer, List<TScanRangeParams>> entry : driverSeqToScanRanges.entrySet()) {
                        ...
                        builder.addMultipleChildren("DriverSequence#" + driverSeq,
    ...
    public String toString() {
        return "FragmentInstance{" + "fragmentId=" + getFragmentId() + ", instanceId=" + DebugUtil.printId(instanceId) +
                ", indexInJob=" + indexInJob + ", indexInFragment=" + indexInFragment + ", address=" + address.getHostname() +
                ":" + address.getPort() + ", execution=" + execution + '}';
    }
