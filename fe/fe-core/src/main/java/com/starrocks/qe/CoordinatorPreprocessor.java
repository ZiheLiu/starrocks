...
public class CoordinatorPreprocessor {
    ...
    private void computeFragmentHosts() throws Exception {
        ...
        for (int i = fragments.size() - 1; i >= 0; --i) {
            ...
            boolean hasBucketShuffle = isBucketShuffleJoin(fragment.getFragmentId().asInt()) && colocatedAssignment != null;

            if (hasColocate || hasBucketShuffle) {
                computeColocatedJoinInstanceParam(colocatedAssignment.getSeqToAddress(),
    ...
    private int getFragmentBucketNum(PlanFragmentId fragmentId) {
        ColocatedBackendSelector.Assignment colocatedAssignment = fragmentExecParamsMap.get(fragmentId).colocatedAssignment;
