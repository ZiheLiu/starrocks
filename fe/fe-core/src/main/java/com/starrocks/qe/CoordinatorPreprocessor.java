...
public class CoordinatorPreprocessor {
    ...
    public CoordinatorPreprocessor(ConnectContext context, JobInformation jobInformation) {
    ...
    CoordinatorPreprocessor(List<PlanFragment> fragments, List<ScanNode> scanNodes) {
        ...
        this.jobInformation = JobInformation.Factory.mockJobInformation(connectContext, fragments, scanNodes);
    ...
    public boolean isUsePipeline() {
        return jobInformation.isEnablePipeline();
    }
    ...
    public TWorkGroup getResourceGroup() {
        return jobInformation.getResourceGroup();
    }
    ...
    private void resetExec() {
        ...
        jobInformation.getFragments().forEach(PlanFragment::reset);
    }
    ...
    private void traceInstance() {
        if (LOG.isDebugEnabled()) {
            ...
            int idx = 0;
            sb.append("query id=").append(DebugUtil.printId(jobInformation.getQueryId())).append(",");
            sb.append("fragment=[");
    ...
    private void computeFragmentHosts() throws Exception {
        ...
        boolean isGatherOutput = jobInformation.getFragments().get(0).getDataPartition() == DataPartition.UNPARTITIONED;

        for (int i = jobInformation.getFragments().size() - 1; i >= 0; --i) {
            PlanFragment fragment = jobInformation.getFragments().get(i);
            FragmentExecParams params = fragmentExecParamsMap.get(fragment.getFragmentId());

            boolean dopAdaptionEnabled = jobInformation.isEnablePipeline() &&
            ...
            if (hasColocate || hasBucketShuffle) {
                computeColocatedJoinInstanceParam(colocatedAssignment.getSeqToWorkerId(),
                        ...
                        parallelExecInstanceNum, pipelineDop, jobInformation.isEnablePipeline(), params);
                computeBucketSeq2InstanceOrdinal(params, colocatedAssignment.getBucketNum());
            } else {
                boolean assignScanRangesPerDriverSeq = jobInformation.isEnablePipeline() &&
    ...
    public class FragmentExecParams {
        ...
        private void toThriftForCommonParams(TExecPlanFragmentParams commonParams,
            ...
            commonParams.params.setQuery_id(jobInformation.getQueryId());
            ...
            commonParams.setQuery_globals(jobInformation.getQueryGlobals());
            if (isEnablePipelineEngine) {
                commonParams.setQuery_options(new TQueryOptions(jobInformation.getQueryOptions()));
            } else {
                commonParams.setQuery_options(jobInformation.getQueryOptions());
            }
            ...
            if (connectContext != null) {
                ...
                if (isEnablePipelineEngine) {
                    ...
                    if (jobInformation.getResourceGroup() != null) {
                        commonParams.setWorkgroup(jobInformation.getResourceGroup());
                    }
                    if (fragment.isUseRuntimeAdaptiveDop()) {
