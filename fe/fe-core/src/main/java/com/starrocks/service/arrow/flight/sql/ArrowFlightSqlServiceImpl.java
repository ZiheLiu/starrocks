...
public class ArrowFlightSqlServiceImpl implements FlightSqlProducer, AutoCloseable {
    ...
    /**
     * Create a prepared statement. Note that, we only support the statement without parameters.
     *
     ...
     * When createing connection in JDBC, they will not be called.
     */
    @Override
    ...
    private FlightInfo getFlightInfoFromQuery(ArrowFlightSqlConnectContext ctx, FlightDescriptor descriptor) {
        try {
            ...
            FlightSql.TicketStatementQuery ticketStatement =
                    FlightSql.TicketStatementQuery.newBuilder().setStatementHandle(handle).build();
            Location endpoint = Location.forGrpcInsecure(worker.getHost(), worker.getBeArrowPort());
