...
TEST(QueryContextManagerTest, testSetWorkgroup) {
    ...
    ASSERT_OK(query_ctx1->init_workgroup_once(wg.get()));
    ASSERT_OK(query_ctx1->init_workgroup_once(wg.get()));              // None-first invocations have no side-effects.
    ASSERT_ERROR(query_ctx_overloaded->init_workgroup_once(wg.get())); // Exceed concurrency_limit.
    ...
    ASSERT_OK(query_ctx2->init_workgroup_once(wg.get()));
    ASSERT_OK(query_ctx2->init_workgroup_once(wg.get())); // None-first invocations have no side-effects.
    ASSERT_EQ(1, wg->num_running_queries());
