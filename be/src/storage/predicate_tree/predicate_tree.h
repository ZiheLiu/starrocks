...
public:
    ...
    using PredicateBaseNodeHelper::evaluate;
...
private:
    void forward_to_non_empty_col_node() {
        if (!is_in_col_nodes()) {
            return;
        }
...
struct CompoundNodeContext {
    template <CompoundNodeType Type>
    const ColumnPredicateMap& cid_to_column_preds(const PredicateCompoundNode<Type>& node) const;
...
public:
    ...
    std::pair<ColumnPredicateMap, PredicateTree> split_and_or_pred() const;
