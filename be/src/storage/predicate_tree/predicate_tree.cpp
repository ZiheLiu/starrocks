...
std::pair<ColumnPredicateMap, PredicateTree> PredicateTree::split_and_or_pred() const {
    ...
    PredicateAndNode or_tree_root;
    for (const auto& or_child : _root.compound_children()) {
        or_tree_root.add_child(std::move(or_child));
    }

    return {std::move(col_pred_map), create(std::move(or_tree_root))};
}
