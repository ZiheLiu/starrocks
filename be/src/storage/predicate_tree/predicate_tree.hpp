...
void PredicateCompoundNode<Type>::partition_copy(Vistor&& cond, PredicateAndNode* true_pred_tree,
    ...
    *true_pred_tree = PredicateAndNode{};
    *false_pred_tree = PredicateAndNode{};
...
inline Status PredicateCompoundNode<CompoundNodeType::AND>::evaluate(CompoundNodeContexts& contexts, const Chunk* chunk,
    ...
    if (!node_ctx.and_context.has_value()) {
        ...
        std::ranges::sort(ctx.non_vec_children, [](const PredicateColumnNode* lhs, const PredicateColumnNode* rhs) {
    ...
    if (contains_true && !ctx.non_vec_children.empty()) {
        ...
        if (ctx.num_evaluate_non_vec_times++ % 32 == 0) {
            ...
            if (max_filtered_rows != filtered_rows[0]) {
                ...
                std::ranges::sort(idx, [&](size_t lhs, size_t rhs) { return filtered_rows[lhs] > filtered_rows[rhs]; });
                std::vector<const PredicateColumnNode*> sorted_non_vec_children(ctx.non_vec_children.size());
                for (int i = 0; i < ctx.non_vec_children.size(); ++i) {
...
template <CompoundNodeType Type>
const ColumnPredicateMap& CompoundNodeContext::cid_to_column_preds(const PredicateCompoundNode<Type>& node) const {
    ...
    for (const auto& [cid, col_children] : node.col_children_map()) {
        for (const auto& child : col_children) {
            cid_to_column_preds[cid].emplace_back(child.col_pred());
        }
    }
