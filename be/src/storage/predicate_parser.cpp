...
ColumnPredicate* PredicateParser::create_column_predicate(const FilterCondition& condition, TypeInfoPtr& type_info,
                                                          ColumnId index) {
    ColumnPredicate* pred = nullptr;
    if ((condition.condition_op == "*=" || condition.condition_op == "=") && condition.condition_values.size() == 1) {
...
ColumnPredicate* OlapPredicateParser::parse_thrift_cond(const FilterCondition& condition) const {
    return t_parse_thrift_cond(condition);
}
...
ColumnPredicate* ConnectorPredicateParser::parse_thrift_cond(const FilterCondition& condition) const {
    return t_parse_thrift_cond(condition);
}
