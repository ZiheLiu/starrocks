...
StatusOr<PredicateTree> ColumnPredicateRewriter::rewrite_predicate(PredicateTree& pred_tree, ObjectPool* pool) {
    ...
    auto status_and_node = root.visit(RewritePredicateTreeVisitor{*this, cid_to_field, pool});
    RETURN_IF_ERROR(status_and_node.status());
    auto& [rewrite_status, new_root] = status_and_node.value();
    ...
    case RewriteStatus::ALWAYS_FALSE:
        ...
        return PredicateTree();
    case RewriteStatus::CHANGED:
        return PredicateTree::create(std::move(new_root.value()));
    case RewriteStatus::UNCHANGED:
        [[fallthrough]];
    default:
...
Status GlobalDictPredicatesRewriter::rewrite_predicate(PredicateTree& pred_tree, ObjectPool* pool) {
    auto root = pred_tree.release_root();
    RETURN_IF_ERROR(root.visit(GlobalDictPredicateTreeVisitor{this, pool}));
...
StatusOr<PredicateTree> ZonemapPredicatesRewriter::rewrite_predicate_tree(ObjectPool* pool, const PredicateTree& src) {
    ASSIGN_OR_RETURN(auto new_pred_root, src.visit(ZonemapPredicatesRewriterVisitor{pool}));
    return PredicateTree::create(std::move(new_pred_root));
}
