...
SegmentIterator::SegmentIterator(std::shared_ptr<Segment> segment, Schema schema, SegmentReadOptions options)
    ...
    std::tie(_cid_to_predicates, _or_pred_tree) = _opts.pred_tree.split_and_or_pred();
...
struct BloomFilterEvaluator {
    Status operator()(const PredicateAndNode& node) {
        if (!used_nodes.contains(&node)) {
            return Status::OK();
        }

        const auto& ctx = pred_tree.compound_node_context(node.id());
        const auto& cid_to_column_preds = ctx.cid_to_column_preds(node);
        for (const auto& [cid, col_preds] : cid_to_column_preds) {
    ...
    Status operator()(const PredicateOrNode& node) {
        if (node.empty() || !used_nodes.contains(&node)) {
            return Status::OK();
        }

        // PredicateOrNode needn't check used_nodes, because a PredicateOrNode is used only when all the children are used.
