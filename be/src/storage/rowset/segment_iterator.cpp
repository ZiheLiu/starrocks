...
void SegmentIterator::_init_column_predicates() {
    ...
    PredicateTreeNode useless_pred_root;
    PredicateTreeNode used_pred_root;
    ...
    PredicateTreeNode expr_pred_root;
    PredicateTreeNode non_expr_pred_root;
        ...
        if constexpr (Type == CompoundNodeType::AND) {
            if (!has_apply_only_del_columns) {
                ...
                for (const auto& cid : del_columns) {
                    if (cid_to_column_preds.contains(cid)) {
                        continue;
                    }
...
Status SegmentIterator::_apply_bitmap_index() {
    ...
    if (new_pred_tree.has_value()) {
        _opts.pred_tree = PredicateTree::create(std::move(new_pred_tree.value()));
    } else {
        _opts.pred_tree = PredicateTree{};
    }
