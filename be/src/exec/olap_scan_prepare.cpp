...
ChunkPredicateBuilder<E>::ChunkPredicateBuilder(const OlapScanConjunctsManagerOptions& opts, CompoundNodeType type,
...
StatusOr<bool> ChunkPredicateBuilder<E>::_normalize_compound_predicate(const Expr* root_expr) {
    ...
    if (TExprOpcode::COMPOUND_AND == root_expr->op()) {
        ChunkPredicateBuilder<RawExprContainer> child_builder(_opts, CompoundNodeType::AND,
                                                              build_raw_expr_containers(root_expr->children()), false);
        ASSIGN_OR_RETURN(const bool normalized, child_builder.parse_conjuncts());
        if (normalized) {
            _child_builders.emplace_back(std::move(child_builder));
...
template <ExprContainer E>
StatusOr<PredicateTree> ChunkPredicateBuilder<E>::get_predicate_tree(PredicateParser* parser,
                                                                     ColumnPredicatePtrs& col_preds_owner) {
    auto compound_node = _type == CompoundNodeType::AND ? PredicateTreeNode{PredicateTreeAndNode()}
                                                        : PredicateTreeNode{PredicateTreeOrNode()};
    ...
    for (size_t i = start_i; i < col_preds_owner.size(); i++) {
        compound_node.visit([&](auto& node) { node.add_child(PredicateTreeColumnNode{col_preds_owner[i].get()}); });
    }

    for (auto& child_builder : _child_builders) {
        ASSIGN_OR_RETURN(auto child_node, child_builder.get_predicate_tree(parser, col_preds_owner));
        compound_node.visit([&](auto& node) { node.add_child(std::move(child_node)); });
    }
...
struct ColumnRangeBuilder {
    template <LogicalType ltype, typename E>
    Status operator()(ChunkPredicateBuilder<E>* parent, const SlotDescriptor* slot,
...
template <ExprContainer E>
Status ChunkPredicateBuilder<E>::_get_column_predicates(PredicateParser* parser, ColumnPredicatePtrs& col_preds_owner) {
...
OlapScanConjunctsManager::OlapScanConjunctsManager(OlapScanConjunctsManagerOptions&& opts)
        : _opts(opts),
          _root_builder(_opts, CompoundNodeType::AND, build_expr_context_containers(*_opts.conjunct_ctxs_ptr), true) {}
...
StatusOr<PredicateTree> OlapScanConjunctsManager::get_predicate_tree(PredicateParser* parser,
                                                                     ColumnPredicatePtrs& col_preds_owner) {
    return _root_builder.get_predicate_tree(parser, col_preds_owner);
}
