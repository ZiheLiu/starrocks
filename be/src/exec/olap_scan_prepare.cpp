...
template <ExprContainer E>
StatusOr<PredicateTreeNode> ChunkPredicateBuilder<E>::get_predicate_tree_root(PredicateParser* parser,
    ...
    for (auto& child_builder : _child_builders) {
        ASSIGN_OR_RETURN(auto child_node, child_builder.get_predicate_tree_root(parser, col_preds_owner));
        compound_node.visit([&](auto& node) { node.add_child(std::move(child_node)); });
    }

    return PredicateTreeNode{std::move(compound_node)};
}
