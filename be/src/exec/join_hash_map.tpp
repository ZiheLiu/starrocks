...
template <LogicalType LT>
const auto& JoinBuildFunc<LT>::get_key_data(const JoinHashTableItems& table_items) {
    ...
    return GetContainer<LT>::get_data(data_column);
}

template <LogicalType LT>
const auto& JoinBuildFunc<LT>::get_raw_key_data(const JoinHashTableItems& table_items) {
    ColumnPtr data_column;
    if (table_items.key_columns[0]->is_nullable()) {
        auto* null_column = ColumnHelper::as_raw_column<NullableColumn>(table_items.key_columns[0]);
        data_column = null_column->data_column();
...
void JoinBuildFunc<LT>::construct_hash_table(RuntimeState* state, JoinHashTableItems* table_items,
    ...
    if (table_items->mode == 6) {
        do_construct_hash_table<6>(state, table_items, probe_state);
        return;
    }
    ...
    // left semi/anti join
    if (table_items->mode == 7) {
        do_construct_hash_table<7>(state, table_items, probe_state);
        return;
    }

    if (table_items->mode == 8) {
        do_construct_hash_table<8>(state, table_items, probe_state);
        return;
    }
    ...
    if (table_items->key_columns[0]->is_nullable()) {
        ...
        if (nullable_column->has_null()) {
            if constexpr (SIMD == 8 && std::is_integral_v<CppType> && sizeof(CppType) == 8) {
                ...
                auto* __restrict set_buckets = table_items->set_buckets.data();
                ...
                for (; i + N <= num_rows; i += N) {
                    ...
                    for (size_t j = 0; j < N; j++) {
                        if (j + 16 < N && null_array[i + j + 16] != 0) {
                            __builtin_prefetch(set_buckets + (probe_buckets_data[j + 16] >> 8));
                        }
                        ...
                        while (true) {
                            if (set_buckets[bucket] == 0) {
                                set_buckets[bucket] = value;
                                set_buckets[start_bucket] |= (fp << 56);
                                break;
                            }
                            if ((set_buckets[bucket] & BLOOM_FILTER_DATA_MASK64) == value) {
                                break;
                            }
                ...
                for (size_t j = 0; i + j < num_rows; j++) {
                    probe_buckets_data[j] = JoinHashMapHelper::calc_bucket_num<CppType>(
                            pdata[i + j], table_items->bucket_size << 8, table_items->log_bucket_size + 8);
                }

                for (size_t j = 0; i < num_rows; i++, j++) {
                    if (i + 16 < num_rows && null_array[i + j + 16] != 0) {
                        __builtin_prefetch(set_buckets + (probe_buckets_data[j + 16] >> 8));
                    }
                ...
                for (size_t i = 1; i < num_rows; i++) {
                    if (null_array[i] == 0) {
                        if constexpr (SIMD == 1) {
                            ...
                            if (const bool bf_matched = ((~(str_firsts[start_bucket].index >> 24)) & fp) == 0;
                                !bf_matched) {
                                while (true) {
                            ...
                            uint32_t bucket = nexts[i];
                            uint32_t probe_times = 1;
                            while (firsts[bucket] != 0 && pdata[firsts[bucket]] != pdata[i]) {
                                bucket = (bucket + probe_times) & bucket_size_mask;
                                probe_times++;
                            }
        ...
        if constexpr (SIMD == 8 && std::is_integral_v<CppType> && sizeof(CppType) == 8) {
            ...
            for (size_t i = 1; i < num_rows; i++) {
                if constexpr (SIMD == 1) {
                    ...
                    if (const bool bf_matched = ((~(str_firsts[start_bucket].index >> 24)) & fp) == 0; !bf_matched) {
                        while (true) {
                            if (str_firsts[bucket].index == 0) {
                                str_firsts[bucket].index = i;
                                if (slice.size < 12) {
...
template <LogicalType LT>
const auto& DirectMappingJoinBuildFunc<LT>::get_key_data(const JoinHashTableItems& table_items) {
    if (table_items.key_columns[0]->is_nullable()) {
        auto* null_column = ColumnHelper::as_raw_column<NullableColumn>(table_items.key_columns[0]);
...
template <LogicalType LT>
const auto& DirectMappingJoinProbeFunc<LT>::get_key_data(const HashTableProbeState& probe_state) {
    if ((*probe_state.key_columns)[0]->is_nullable()) {
        auto* nullable_column = ColumnHelper::as_raw_column<NullableColumn>((*probe_state.key_columns)[0]);
...
void JoinProbeFunc<LT>::lookup_init(const JoinHashTableItems& table_items, HashTableProbeState* probe_state) {
    if (table_items.mode == 1) {
        ...
        do_lookup_init<8>(table_items, probe_state);
    } else {
...
void JoinProbeFunc<LT>::do_lookup_init(const JoinHashTableItems& table_items, HashTableProbeState* probe_state) {
    ...
    if constexpr (SIMD == 1 || SIMD == 8 || SIMD == 6 || SIMD == 16) {
...
template <LogicalType LT>
const auto& JoinProbeFunc<LT>::get_key_data(const HashTableProbeState& probe_state) {
    if ((*probe_state.key_columns)[0]->is_nullable()) {
        auto* nullable_column = ColumnHelper::as_raw_column<NullableColumn>((*probe_state.key_columns)[0]);
        return ColumnHelper::as_raw_column<ColumnType>(nullable_column->data_column())->get_data();
    }
...
void JoinHashMap<LT, BuildFunc, ProbeFunc>::probe_prepare(RuntimeState* state) {
    ...
    _probe_state->probe_buckets.resize(chunk_size + 8);
...
void JoinHashMap<LT, BuildFunc, ProbeFunc>::_search_ht_impl(RuntimeState* state, const auto& build_data,
...
#define RETURN_IF_CHUNK_FULL3()                                  \
    if (UNLIKELY(match_count > state->chunk_size())) {           \
        _probe_state->cur_probe_index = i;                       \
        _probe_state->next[i] = probe_bucket;                    \
...
template <LogicalType LT, class BuildFunc, class ProbeFunc>
template <bool first_probe>
void JoinHashMap<LT, BuildFunc, ProbeFunc>::_probe_coroutine(RuntimeState* state, const auto& build_data,
...
template <LogicalType LT, class BuildFunc, class ProbeFunc>
template <bool first_probe>
void JoinHashMap<LT, BuildFunc, ProbeFunc>::_probe_from_ht(RuntimeState* state, const auto& build_data,
                                                           const Buffer<CppType>& probe_data) {
...
template <LogicalType LT, class BuildFunc, class ProbeFunc>
void JoinHashMap<LT, BuildFunc, ProbeFunc>::_do_probe_from_ht_mode6_first(RuntimeState* state, const auto& build_data,
                                                                          const Buffer<CppType>& probe_data) {
...
void JoinHashMap<LT, BuildFunc, ProbeFunc>::_do_probe_from_ht_mode6(RuntimeState* state, const auto& build_data,
    ...
    auto process_opt = [&](const uint32_t start_match_count, uint32_t probe_index, uint32_t cached_idx) {
        const auto* __restrict cached_nexts = _table_items->cached_nexts.data();

        cached_idx &= 0x7FFF'FFFFull;
        ...
        for (cached_idx++; (cached_nexts[cached_idx] & 0x8000'0000ull) == 0; cached_idx++) {
    ...
    for (; i < res_buckets_len; i++) {
        ...
        if (first_next & 0x8000'0000ull) {
            ...
            if (first_next == 0) {
                ...
                if (UNLIKELY(match_count > state->chunk_size())) {
                    _probe_state->probe_index[start_match_count] = probe_index;
                    _probe_state->cur_probe_index = i;
                ...
                uint32_t num_cached_nexts = _table_items->num_cached_nexts;
                auto* cached_nexts = _table_items->cached_nexts.data();
                ...
                do {
                    cached_nexts[num_cached_nexts] = cur_build_index;
                    num_cached_nexts++;
                ...
                if (process_opt(start_match_count, probe_index, new_first_next)) {
                    return;
                }
            }
        }
...
void JoinHashMap<LT, BuildFunc, ProbeFunc>::_probe_from_ht_for_left_outer_join(RuntimeState* state,
    ...
    if (_table_items->mode == 6) {
        _do_probe_from_ht_for_left_outer_join<first_probe, false, false, 6>(state, build_data, probe_data);
        return;
    }

    if (_table_items->mode == 16) {
        _do_probe_from_ht_for_left_outer_join<first_probe, false, false, 16>(state, build_data, probe_data);
        return;
    }
...
void JoinHashMap<LT, BuildFunc, ProbeFunc>::_do_probe_from_ht_for_left_outer_join(RuntimeState* state,
    ...
    if constexpr (SIMD == 6 || SIMD == 16) {
        _do_probe_from_ht_for_left_outer_join_mode6<first_probe, SIMD>(state, build_data, probe_data);
        return;
    }
...
void JoinHashMap<LT, BuildFunc, ProbeFunc>::_probe_from_ht_for_left_semi_join(RuntimeState* state,
    ...
    } else if (_table_items->mode == 7) {
        _do_probe_from_ht_for_left_semi_join<first_probe, false, 7>(state, build_data, probe_data);
    } else if (_table_items->mode == 8) {
        _do_probe_from_ht_for_left_semi_join<first_probe, false, 8>(state, build_data, probe_data);
...
void JoinHashMap<LT, BuildFunc, ProbeFunc>::_do_probe_from_ht_for_left_semi_join(RuntimeState* state,
    ...
    if constexpr (MODE == 3 && std::is_integral_v<CppType> && (sizeof(CppType) == 4 || sizeof(CppType) == 8)) {
    ...
    } else if constexpr (MODE == 8 && std::is_integral_v<CppType> && sizeof(CppType) == 8) {
        const uint32_t bucket_size_mask = _table_items->bucket_size - 1;
        const auto* buckets = _table_items->set_buckets.data();
        const auto* probe_buckets = _probe_state->buckets.data();

        for (uint32_t i = 0; i < probe_row_count; i++) {
            if (i + 16 < probe_row_count) {
                __builtin_prefetch(buckets + (probe_buckets[i + 16] >> 8));
            }

            const uint32_t hash = probe_buckets[i];
            uint32_t bucket = hash >> 8;

            const uint64_t cur_bf = BLOOM_FILTERS[hash & 0xFF];
            if (const bool matched = ((~(buckets[bucket] >> 56)) & cur_bf) == 0; !matched) {
                continue;
            }
            ...
            uint32_t probe_times = 1;
            while (true) {
                const auto build_bucket = buckets[bucket];
                if (build_bucket == 0) {
                    break;
                }
        ...
        for (uint32_t i = 0; i < probe_row_count; i++) {
            ...
            uint32_t probe_times = 1;
            while (true) {
                const uint32_t build_idx = build_buckets[probe_bucket];
                if (build_idx == 0) {
                    break;
                }
                if (build_data[build_idx] == probe_key) {
                    dst_probe_indexes[match_count] = i;
                    match_count++;
                    break;
                }
...
void JoinHashMap<LT, BuildFunc, ProbeFunc>::_probe_from_ht_for_left_anti_join(RuntimeState* state,
    ...
    if (_table_items->mode == 3) {
        ...
        _do_probe_from_ht_for_left_anti_join<first_probe, false, 7>(state, build_data, probe_data);
    } else if (_table_items->mode == 8) {
        _do_probe_from_ht_for_left_anti_join<first_probe, false, 8>(state, build_data, probe_data);
    } else if (_table_items->mode == 1) {
        if (_table_items->no_conflicts) {
    ...
    if (_table_items->join_type == TJoinOp::NULL_AWARE_LEFT_ANTI_JOIN && _probe_state->null_array != nullptr) {
        ...
        if constexpr (SIMD == 3) {
            ...
            for (uint32_t i = 0; i < probe_row_count; i++) {
                ...
                uint32_t probe_times = 1;
                while (true) {
                    const auto build_idx = buckets[bucket];
                    if (build_idx == 0) {
                        probe_indexes[match_count] = i;
                        match_count++;
                        break;
