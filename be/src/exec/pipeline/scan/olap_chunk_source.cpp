...
Status OlapChunkSource::_init_reader_params(const std::vector<std::unique_ptr<OlapScanRange>>& key_ranges) {
    ...
    LOG(WARNING) << "[TEST] [num_not_pushdown_conjuncts=" << not_pushdown_conjuncts.size() << "]";

    for (auto* expr : not_pushdown_conjuncts) {
        expr->root()->for_each_slot_id([&conjuncts_slot_ids](SlotId id) {
            LOG(WARNING) << "[TEST] [slot_id=" << id << "]";
            conjuncts_slot_ids.insert(id);
        });
    }
