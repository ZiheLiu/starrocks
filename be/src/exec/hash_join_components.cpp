...
private:
    ...
    size_t _probe_partition_index = 0;
};
...
private:
    size_t _estimated_build_row_key_bytes(const HashTableParam& param) const;
    size_t _estimated_build_row_output_bytes(const HashTableParam& param) const;
    size_t _estimated_probe_row_bytes(const HashTableParam& param) const;
...
size_t AdaptivePartitionHashJoinBuilder::_estimated_build_row_key_bytes(const HashTableParam& param) const {
...
size_t AdaptivePartitionHashJoinBuilder::_estimated_build_row_output_bytes(const HashTableParam& param) const {
    size_t estimated_each_row = 0;
...
size_t AdaptivePartitionHashJoinBuilder::_estimated_probe_row_bytes(const HashTableParam& param) const {
    size_t size = 0;
...
void AdaptivePartitionHashJoinBuilder::_adjust_partition_rows(size_t build_row_size) {
    const size_t build_row_output_bytes =
            build_row_size > _build_row_key_bytes ? build_row_size - _build_row_key_bytes : 4UL;
    ...
    const auto l2_benefit =
            estimate_benefit.operator()<CacheLevel::L2, CacheLevel::L3>(build_row_output_bytes, _build_row_key_bytes);
    const auto l3_benefit = estimate_benefit.operator()<CacheLevel::L3, CacheLevel::MEMORY>(build_row_output_bytes,
                                                                                            _build_row_key_bytes);

    if (_probe_row_shuffle_cost < l3_benefit) {
        ...
        _partition_join_l3_max_rows += _partition_join_l3_max_rows / 2;

        if (_probe_row_shuffle_cost < l2_benefit) {
            ...
            _partition_join_l2_max_rows += _partition_join_l2_max_rows / 2;
        }
    ...
    VLOG_OPERATOR << "TRACE:"
                  ...
                  << "[build_row_key_bytes=" << _build_row_key_bytes << "] "
...
void AdaptivePartitionHashJoinBuilder::_init_partition_nums(const HashTableParam& param) {
    _partition_num = config::partition_hash_join_num == 0 ? 16 : std::abs(config::partition_hash_join_num);

    _probe_row_shuffle_cost = _estimate_cost_by_bytes<CacheLevel::L3>(_estimated_probe_row_bytes(param));
    _build_row_key_bytes = _estimated_build_row_key_bytes(param);

    const size_t estimated_build_row_bytes = _estimated_build_row_output_bytes(param) + _build_row_key_bytes;
...
Status AdaptivePartitionHashJoinBuilder::do_append_chunk(const ChunkPtr& chunk) {
    if (_partition_num > 1 && config::partition_hash_join_num <= 0 &&
        !_need_partition_join_for_append(hash_table_row_count())) {
...
Status AdaptivePartitionHashJoinBuilder::build(RuntimeState* state) {
    ...
    if (_partition_num > 1 && config::partition_hash_join_num <= 0 &&
        !_need_partition_join_for_build(hash_table_row_count())) {
        RETURN_IF_ERROR(_convert_to_single_partition());
    }
