...
void BinaryColumnBase<T>::serialize_batch(uint8_t* dst, Buffer<uint32_t>& slice_sizes, size_t chunk_size,
    ...
    for (size_t i = 0; i < chunk_size; ++i) {
        ...
        const auto binary_size = static_cast<uint32_t>(_offsets[i + 1] - offset);
        auto* cur_dst = dst + i * max_one_row_size + slice_sizes[i];
        strings::memcpy_inlined(cur_dst, &binary_size, sizeof(uint32_t));
        strings::memcpy_inlined(cur_dst + sizeof(uint32_t), &_bytes[offset], binary_size);
    }
...
void BinaryColumnBase<T>::serialize_batch_with_null_masks(uint8_t* dst, Buffer<uint32_t>& slice_sizes,
    ...
    if (!has_null) {
        for (size_t i = 0; i < chunk_size; ++i) {
            ...
            strings::memcpy_inlined(cur_dst + sizeof(bool) + sizeof(uint32_t), &_bytes[offset], binary_size);
        }
