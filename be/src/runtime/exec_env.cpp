...
void ExecEnv::try_release_resource_before_core_dump() {
    ...
    if (need_release("connector_scan_executor")) {
        if (_workgroup_manager != nullptr) {
            _workgroup_manager->for_each_executors(
                    [](auto& executors) { executors.connector_scan_executor()->close(); });
        }
        LOG(INFO) << "close connector scan executor";
    }
    if (need_release("olap_scan_executor")) {
    ...
    if (need_release("wg_driver_executor")) {
        if (_workgroup_manager != nullptr) {
