    ...
    static inline arrow::Status convert(const ColumnPtr& column, int start_idx, int end_idx,
        ...
        if constexpr (is_nullable) {
            ...
            if constexpr (lt_is_string<LT> || lt_is_binary<LT>) {
                ...
                // BITMAP,PERCENTILE,VARIANT are always converted to utf8 with null values, which is the same as MySQL output.
                for (auto i = start_idx; i < end_idx; ++i) {
                    ARROW_RETURN_NOT_OK(builder->AppendNull());
                }
            } else {
...
static const std::unordered_map<int32_t, StarRocksToArrowConvertFunc> global_starrocks_to_arrow_conv_table{
        ...
        STARROCKS_TO_ARROW_CONV_ENTRY_R(ArrowTypeId::BINARY, TYPE_VARBINARY, TYPE_OBJECT, TYPE_PERCENTILE),
